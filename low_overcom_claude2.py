...
[조건 4] 아래꼬리 음봉 + 다음날 양봉 종가 > 음봉 고가 → 시그널
          단, (양봉고가 - 음봉종가) / 음봉종가 >= 10% 이면 시그널 제외
[조건 5] 매수가: diff = 양봉고가 - 음봉저가
          1차=음봉저가+diff×4/4(양봉고가), 2차=×3/4, 3차=×2/4, 4차=×1/4
          비율 1:2:3:4, 총 300만원
[조건 6] 익절: 평균매수가 × (1 + tp%) 달성 시 즉시 매도
[조건 7] 손절: 종가 < 음봉 저가 즉시 손절

[자동모드] MA장기(120/240) × MA단기(20/40/없음) × MA하락(5/3) × 익절률(5/7/10%) = 36케이스
"""
...
COLORS = {
    ...
    "buy1": "#00E676", "buy2": "#69F0AE", "buy3": "#B9F6CA", "buy4": "#E8F5E9",
    "tp": "#2196F3",   "sl": "#F44336",
}
...
AUTO_CASES = [
    (lma, sma, dma, tp)
    for lma in [120, 240]
    for sma in [20, 40, None]   # None = 단기MA 조건 미적용
    for dma in [5, 3]
    for tp  in [5, 7, 10]
]
...
def _analyze(days, long_ma_p, short_ma_p, decline_ma_p, tp_pct):
    ...
    조건4: 전전봉=아래꼬리음봉, 전봉=양봉(종가>음봉고가) → 시그널
           단, (양봉고가-음봉종가)/음봉종가 >= 10% 이면 제외
    조건5: diff=양봉고가-음봉저가
           1차=diff×4/4(양봉고가), 2차=×3/4, 3차=×2/4, 4차=×1/4 / 비율 1:2:3:4
    조건6: 익절 tp_pct%
    조건7: 손절 음봉저가
    """
    n=len(days)
    _ma_periods=[p for p in [long_ma_p, short_ma_p, decline_ma_p] if p is not None]
    if n<max(_ma_periods)+10: return [],[],[]

    closes=[d["close"] for d in days]
    lmv=_ma(closes,long_ma_p)
    smv=_ma(closes,short_ma_p) if short_ma_p is not None else None
    ...
    bear_dt=bull_dt=""
    bear_high=bear_low=bear_close=bull_high=bull_close=0.0
    buy1_p=buy2_p=buy3_p=buy4_p=sl_p=0.0
    tot_cost=0.0; tot_qty=0
    buy1_done=buy2_done=buy3_done=buy4_done=False
    first_buy_dt=""

    r_sum=sum(BUY_RATIO)  # 1+2+3+4=10
    amt1=int(TOTAL_INVEST*BUY_RATIO[0]/r_sum)
    amt2=int(TOTAL_INVEST*BUY_RATIO[1]/r_sum)
    amt3=int(TOTAL_INVEST*BUY_RATIO[2]/r_sum)
    amt4=int(TOTAL_INVEST*BUY_RATIO[3]/r_sum)

    def _reset():
        nonlocal state,bear_dt,bull_dt,bear_high,bear_low,bear_close,bull_high,bull_close
        nonlocal buy1_p,buy2_p,buy3_p,buy4_p,sl_p
        nonlocal tot_cost,tot_qty,buy1_done,buy2_done,buy3_done,buy4_done,first_buy_dt
        state=ST_IDLE; bear_dt=bull_dt=""
        bear_high=bear_low=bear_close=bull_high=bull_close=0.0
        buy1_p=buy2_p=buy3_p=buy4_p=sl_p=0.0
        tot_cost=0.0; tot_qty=0
        buy1_done=buy2_done=buy3_done=buy4_done=False; first_buy_dt=""
    ...
    def _mk_trade(sell_dt, sell_p, sell_type):
        ...
        trades.append({"bear_dt":bear_dt,"bull_dt":bull_dt,
            "buy1_p":buy1_p,"buy2_p":buy2_p,"buy3_p":buy3_p,"buy4_p":buy4_p,"sl_p":sl_p,
    ...
    for i in range(2,n):
        ...
        # ── IDLE: 시그널 탐색 ──
        if state==ST_IDLE:
            if lmv[i] is None or dmv[i] is None: continue
            if smv is not None and smv[i] is None: continue
            if not _ma_consec_up(lmv,i,5): continue
            if smv is not None and not _ma_up_today(smv,i): continue
            ...
            if p1["low"]<p2["low"]: continue  # 양봉저가 < 음봉저가 → 제외
            # 과도 상승 필터: (양봉고가 - 음봉종가) / 음봉종가 >= 10% 제외
            if p2["close"]>0 and (p1["high"]-p2["close"])/p2["close"]>=0.10: continue
            # 시그널 확정
            bear_dt=p2["dt"]; bull_dt=p1["dt"]
            bear_high=p2["high"]; bear_low=p2["low"]; bear_close=p2["close"]
            bull_high=p1["high"]; bull_close=p1["close"]
            # diff = 양봉고가 - 음봉저가
            diff=bull_high-bear_low
            buy1_p=bear_low+diff*4/4  # = bull_high (1차: 양봉 윗꼬리)
            buy2_p=bear_low+diff*3/4
            buy3_p=bear_low+diff*2/4
            buy4_p=bear_low+diff*1/4
            sl_p=bear_low
            signals.append({"bear_dt":bear_dt,"bull_dt":bull_dt,"entry_dt":d["dt"],
                "bear_high":bear_high,"bear_low":bear_low,"bear_close":bear_close,
                "bull_high":bull_high,"bull_close":bull_close,
                "buy1_p":buy1_p,"buy2_p":buy2_p,"buy3_p":buy3_p,"buy4_p":buy4_p,"sl_p":sl_p})
            tlog.append({"action":"시그널","dt":d["dt"],"price":0,"qty":0})
            state=ST_BUY_WAIT
            buy1_done=buy2_done=buy3_done=buy4_done=False
            tot_cost=0.0; tot_qty=0; first_buy_dt=""
            # fall-through → 시그널 당일 즉시 BUY_WAIT 체결 체크
        ...
        if state==ST_BUY_WAIT:
            ...
            if not buy4_done and l<=buy4_p:
                fp=min(o,buy4_p); q=int(amt4/fp) if fp>0 else 0
                if q>0:
                    buy4_done=True; tot_cost+=fp*q; tot_qty+=q
                    if not first_buy_dt: first_buy_dt=d["dt"]
                    tlog.append({"action":"매수4차","dt":d["dt"],"price":fp,"qty":q})
...
def _enrich(days, long_ma_p, short_ma_p, decline_ma_p):
    ...
    dmv=_ma(closes,decline_ma_p)
    def _row(i, d):
        r={**d, f"ma{long_ma_p}":lmv[i], f"ma{decline_ma_p}":dmv[i]}
        if smv is not None: r[f"ma{short_ma_p}"]=smv[i]
        return r
    return [_row(i,d) for i,d in enumerate(days)]
...
def _trade_df(trades):
    ...
    for i,t in enumerate(trades):
        rows.append({"No":i+1,"음봉일":t["bear_dt"],"양봉일":t["bull_dt"],
            ...
            "3차매수가":f"{t['buy3_p']:,.0f}","4차매수가":f"{t.get('buy4_p',0):,.0f}",
            "손절가":f"{t['sl_p']:,.0f}",
...
def _chart(en, signals, trades, tlog, name,
    ...
    if short_ma_p is not None:
        ma_styles.insert(1, (f"ma{short_ma_p}", f"MA{short_ma_p}",
                             COLORS.get(f"ma{short_ma_p}","#FFD600"), 1.2, "solid"))
    ...
    for s in signals:
        ...
        for price,clr,dash in [
            ...
            (s.get("buy4_p",0),COLORS["buy4"],"longdashdot"),
    ...
    fig.update_layout(
        title=f"📊 {name} — MA{long_ma_p}↑ {'MA'+str(short_ma_p)+'↑ ' if short_ma_p else ''}MA{decline_ma_p}↓{period_str}",
...
def main():
    ...
    with st.sidebar:
        ...
        if ma_mode=="수동":
            ...
            short_ma_p  = st.selectbox("조건2 — 단기MA (오늘 상승)",
                                        [20, 40, None],
                                        format_func=lambda x: f"MA{x}" if x else "선택안함")
        ...
        st.markdown("---")
        st.caption(f"총 투자금 {TOTAL_INVEST:,}원 / 1차:2차:3차:4차 = 1:2:3:4")
    ...
    if scan:
            ...
            try:
                ...
                if ma_mode=="수동":
                    ...
                    for ci,(lma,sma,dma,tp) in enumerate(AUTO_CASES):
                        _sma_str = f"MA{sma}↑ " if sma is not None else ""
                        pg.progress((ci+1)/len(AUTO_CASES),
                                    f"케이스 {ci+1}/{len(AUTO_CASES)}: MA{lma}↑ {_sma_str}MA{dma}↓ +{tp}%")
                        ...
                        s=_stats(trd_f)
                        label=f"MA{lma}↑ {_sma_str}MA{dma}↓ 익절{tp}%"
        ...
        # ══ 다중 종목 ══════════════════════════════════════════
            ...
            if ma_mode=="수동":
                ...
                st.info(f"📂 {len(qs)}개 종목 × 36케이스 분석 중...")
                days_map={}; errors=[]
                pg0=st.progress(0)
                for idx,q in enumerate(qs):
                ...
                pg=st.progress(0)
                for ci,(lma,sma,dma,tp) in enumerate(AUTO_CASES):
                    _sma_str2 = f"MA{sma}↑ " if sma is not None else ""
                    pg.progress((ci+1)/len(AUTO_CASES),
                                f"케이스 {ci+1}/36: MA{lma}↑ {_sma_str2}MA{dma}↓ +{tp}%")
                    all_trd=[]
                    ...
                    s=_stats(all_trd)
                    label=f"MA{lma}↑ {_sma_str2}MA{dma}↓ 익절{tp}%"
...
**[조건4]** 과도 상승 필터
- (양봉고가 - 음봉종가) / 음봉종가 >= 10% 이면 시그널 제외

**[조건5]** 시그널 당일부터 진입
- diff = 양봉고가 − 음봉저가
- 1차 = 음봉저가 + diff×4/4 = 양봉고가 (≈30만원)
- 2차 = 음봉저가 + diff×3/4 (≈60만원)
- 3차 = 음봉저가 + diff×2/4 (≈90만원)
- 4차 = 음봉저가 + diff×1/4 (≈120만원)
- 총 300만원 / 비율 1:2:3:4
