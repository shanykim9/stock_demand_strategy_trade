"""
pattern_scan6_final.py — 5MA 반등 + 40MA 상승 조건 + 고가15% 익절 (v6 final)
═══════════════════════════════════════════════════════════════
실행: streamlit run pattern_scan6_final.py

[v6 변경사항]
1. 트리거 조건 추가: 40MA 2일 연속 상승
2. 매수 시그널 조건 추가: 40MA 1일 상승 (오늘 40MA > 어제 40MA)
3. 익절 조건 추가: 매수 후 고가 기준 15% 이상 상승 후 5MA 하락 → 다음날 시가 매도
...
[3] 트리거 (v6 변경)
    양봉 + 거래량>=vol_threshold + 장기MA상승 + 40MA 2일연속 상승
...
[8] 매수 시그널 (v6 변경)
    5MA 하락→상승 + 양봉 + 40MA 상승 (오늘 40MA > 어제 40MA)

[9] 매수
    매수 시그널 다음날 시가에 1차 매수 (투자금 전액)

[10] 익절
    1. (기존) 종가 >= 5MA × 1.12 양봉 → 다음날 시가 매도
    2. (v6 추가) 매수 후 고가 기준 15% 이상 상승한 적 있고, 5MA 하락 → 다음날 시가 매도
...
COLORS = {
    "bull": "#E53935", "bear": "#1E88E5", "bv": "#EF9A9A", "bev": "#90CAF9",
    ...
    "ma120": "#26A69A", "ma240": "#FF7043", "ma5": "#FBC02D", "ma40": "#9C27B0",
...
CACHE_DIR = Path(".cache/pattern_scan6"); CACHE_DIR.mkdir(parents=True, exist_ok=True)
...
def _guides(lo, up):
    r = up - lo
    return {"lo": lo, "up": up, "rng": r, "p14": lo + r * 0.25, "mid": lo + r * 0.5,
            "p34": lo + r * 0.75, "p58": lo + r * 0.625, "p78": lo + r * 0.875}

# ── 핵심 분석 함수 (v6 final) ──────────────────────────────────────
def _analyze(days, ma_period, vol_thr, invest, price_ref, max_watch, use_bear_high, stop_loss_pct):
    n = len(days)
    if n < 42: return [], [], []  # 40MA 계산을 위해 최소 42봉 필요
    closes = [d["close"] for d in days]
    vols = [d["volume"] for d in days]
    mv = _ma(closes, ma_period)   # 장기 MA (120 or 240)
    mv5 = _ma(closes, 5)          # 5MA: 매수시그널/익절 판단
    mv40 = _ma(closes, 40)        # 40MA: 트리거/매수시그널 추가 조건

    ma_n = 5 if ma_period == 240 else 10  # 장기MA 연속상승 기준일수
    ...
    det_info = {}; signal_idx = -1; signal_dt = ""
    avg_p = 0.0; tot_cost = 0.0; tot_qty = 0; buy_dt = ""
    max_high_after_buy = 0.0  # 매수 후 일봉 고가 최고값 추적

    def _reset():
        nonlocal state, ti_idx, b_lo, b_up, g, peak_h, det_info
        nonlocal signal_idx, signal_dt, avg_p, tot_cost, tot_qty, buy_dt, max_high_after_buy
        state = ST_IDLE; ti_idx = 0; b_lo = b_up = 0.0; g = {}; peak_h = 0.0; det_info = {}
        signal_idx = -1; signal_dt = ""; avg_p = 0.0; tot_cost = 0.0; tot_qty = 0
        buy_dt = ""; max_high_after_buy = 0.0
    ...
    def _mk_trade(sell_dt, sell_p, sell_type):
        ...
        trades.append({
            "trigger_dt": days[ti_idx]["dt"], "signal_dt": signal_dt,
    ...
    for i in range(1, n):
        ...
        if state == ST_POS:
            # 보유 중 일봉 고가 최고값 갱신
            max_high_after_buy = max(max_high_after_buy, h)

            # 익절1: 종가 >= 5MA × 1.12 이고 양봉 → 다음날 시가 매도
            if bull and mv5[i] is not None and c >= mv5[i] * 1.12:
                if i + 1 < n:
                    sell_p = days[i + 1]["open"]; sell_dt = days[i + 1]["dt"]
                else:
                    sell_p = c; sell_dt = d["dt"]
                tlog.append({"trigger_dt": days[ti_idx]["dt"], "action": "익절(5MA이격)", "dt": sell_dt, "price": sell_p, "qty": tot_qty})
                _mk_trade(sell_dt, sell_p, "익절(5MA이격)")
                _reset()
                # 익절 후 당일 IDLE로 전환하여 트리거 재탐색 (continue 없이 아래 로직 실행)

            # 익절2: 매수 후 고가 기준 15% 이상 상승한 적 있고, 5MA 하락 → 다음날 시가 매도
            if state == ST_POS and avg_p > 0 and max_high_after_buy >= avg_p * 1.15:
                if i >= 1 and mv5[i] is not None and mv5[i - 1] is not None:
                    if mv5[i] < mv5[i - 1]:
                        if i + 1 < n:
                            sell_p = days[i + 1]["open"]; sell_dt = days[i + 1]["dt"]
                        else:
                            sell_p = c; sell_dt = d["dt"]
                        tlog.append({"trigger_dt": days[ti_idx]["dt"], "action": "익절(고가15%+5MA하락)", "dt": sell_dt, "price": sell_p, "qty": tot_qty})
                        _mk_trade(sell_dt, sell_p, "익절(고가15%+5MA하락)")
                        _reset()
        ...
        if state == ST_BUY_WAIT:
            if i > signal_idx:
                ...
                if qty > 0:
                    ...
                    max_high_after_buy = buy_p  # 매수 시점 고가 추적 시작
        ...
        if state == ST_PULL:
            ...
            # 매수 시그널: 5MA 하락→상승 전환 양봉 + 40MA 상승 (v6 추가)
            if (i >= 2
                    and mv5[i] is not None and mv5[i - 1] is not None and mv5[i - 2] is not None
                    and mv40[i] is not None and mv40[i - 1] is not None):
                ma5_v_turn = mv5[i - 1] < mv5[i - 2] and mv5[i] > mv5[i - 1]  # 5MA V턴
                ma40_up = mv40[i] > mv40[i - 1]                                  # 40MA 상승
                if ma5_v_turn and bull and ma40_up:
                    signal_idx = i; signal_dt = d["dt"]
                    tlog.append({"trigger_dt": days[ti_idx]["dt"], "action": "매수시그널(5MA반등+40MA상승)", "dt": d["dt"], "price": c, "qty": 0})
                    state = ST_BUY_WAIT
            continue
        ...
        if state == ST_TRACK:
            ...
            b_up = max(peak_h, h) if use_bear_high else peak_h
        ...
        # ── ST_IDLE: 트리거 탐색 ─────────────────────────────────────
        if state == ST_IDLE:
            if mv[i] is None or mv40[i] is None or mv40[i - 1] is None: continue
            if not _ma_up(mv, i, ma_n): continue          # 장기MA N일 연속 상승
            if not _ma_up(mv40, i, 2): continue           # 40MA 2일 연속 상승 (v6 추가)
            if not bull: continue                          # 양봉
            if vols[i] < vol_thr: continue                # 거래량 기준 이상
            b_lo = _base_lower(days, i); peak_h = d["high"]
            signal_idx = -1; signal_dt = ""; avg_p = 0.0; tot_cost = 0.0; tot_qty = 0
            buy_dt = ""; max_high_after_buy = 0.0; det_info = {}
            ti_idx = i; state = ST_TRACK
            continue
...
def _enrich(days, ma_p):
    ...
    m = _ma(c, ma_p); m5 = _ma(c, 5); m40 = _ma(c, 40)
    return [{**d, f"ma{ma_p}": m[i], "ma5": m5[i], "ma40": m40[i]} for i, d in enumerate(days)]
...
def _chart(en, dets, trades, tlog, name, ma_p, pd_=0):
    ...
    if "ma40" in df.columns:
        fig.add_trace(go.Scatter(x=df["dt"], y=df["ma40"], name="MA40", line=dict(color=COLORS["ma40"], width=1.2), hoverinfo="skip"), row=1, col=1)
    ...
    for det in dets:
        ...
        for v, cl, da in [
            (det["base_upper"], COLORS["upper"], "dot"), (det["base_lower"], COLORS["lower"], "dot"),
    ...
    for e in tlog:
        ...
        if "매수시그널" in act:
            ...
            fig.add_trace(go.Scatter(x=[edt], y=[e["price"] * 1.02], mode="markers", showlegend=False,
                ...
                hovertemplate=f"%{{x|%Y-%m-%d}}<br>{act}<br>{e['price']:,.0f}<extra></extra>"), row=1, col=1)
    fig.update_layout(
        title=f"📊 {name} — 5MA 반등 v6 final (MA{ma_p}+MA40)", template="plotly_dark", height=680,
...
def _r1(nm, tk, en, dets, trd, tlog, map_, pd_, ksuf=""):
    ...
    st.plotly_chart(fig, use_container_width=True, key=f"c6{uk}",
        config={"displayModeBar": True, "displaylogo": False, "scrollZoom": True,
                "modeBarButtonsToRemove": ["lasso2d", "select2d", "autoScale2d", "toggleSpikelines"]})
    if trd:
        st.markdown("#### 📋 트레이드 상세")
        st.dataframe(_sdf(trd), use_container_width=True, hide_index=True, key=f"t6{uk}")
        csv = _sdf(trd).to_csv(index=False, encoding="utf-8-sig").encode("utf-8-sig")
        st.download_button("📥 CSV", csv, f"{nm}_scan6.csv", "text/csv", key=f"d6{uk}")
...
def _rm(results, map_, pd_, ksuf=""):
    ...
    for it in results:
        ...
        ov.append({"종목": f"{it['name']}({it['tk']})", "패턴": len(it.get("dets", [])),
                   ...
                   "수익금": f"{s['tp']:+,.0f}", "손실금": f"{s['tl']:,.0f}", "순손익": f"{s['pnl']:+,.0f}"})
    ...
    if ov:
        ...
        st.download_button("📥 종합CSV", csv, "scan6_summary.csv", "text/csv", key=f"gcsv{ksuf}")
...
def _auto_run(token, queries, infos, days_map, vol_pcts, ma_list, invest, price_ref, max_watch, use_bear_high, cd, stop_loss_pct):
    ...
    for ci, (ma_p, vp) in enumerate(combos):
        ...
        rows.append({"MA": f"MA{ma_p}", "거래비율": f"{vp}%",
            ...
            "평균수익률": f"{s['ar']:+.1f}%",
            "누적수익": f"{s['tp']:+,.0f}", "누적손실": f"{s['tl']:,.0f}", "누적손익": f"{s['pnl']:+,.0f}"})
    pg.empty()
...
# ── 메인 ──────────────────────────────────────────────────────────
def main():
    st.set_page_config(page_title="패턴 스캔 v6", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
    st.markdown(_CSS, unsafe_allow_html=True)
    with st.sidebar:
        st.markdown("## ⚙️ 5MA 반등 스캔 v6")
        ...
        if not auto_mode:
            ...
            auto_pcts = st.multiselect("거래량 비율(%)", list(range(10, 110, 10)), default=[20, 30, 40, 50])
        ...
        use_bear_high = st.checkbox("상한 확정 시 음봉고가 포함", value=True,
                                    help="OFF: peak_high만 / ON: max(peak, 음봉고가)")
        max_pages = st.slider("OHLCV 페이지", 10, 80, 40, 5)
        st.markdown("---")
        st.markdown(f"""<div style="font-size:.72rem;color:#888;line-height:1.8">
        <b style="color:#FFD600">★</b> 트리거: 양봉+거래량+MA상승+<b style="color:{COLORS['ma40']}">40MA 2일↑</b><br>
        <b style="color:#FF9800">●</b> 눌림시작: ref &lt; 3/4선<br>
        <b style="color:{COLORS['buy']}">▲</b> 매수시그널: 5MA V턴 양봉 + <b style="color:{COLORS['ma40']}">40MA↑</b><br>
        <b style="color:{COLORS['buy']}">▲</b> 매수: 시그널 다음날 시가<br>
        <b style="color:{COLORS['tp']}">▼</b> 익절①: 종가 ≥ 5MA×1.12 양봉 → 다음날 시가<br>
        <b style="color:{COLORS['tp']}">▼</b> 익절②: 고가 15%↑ 후 5MA 하락 → 다음날 시가<br>
    ...
    if scan:
        ...
        if not _appkey or not _secretkey:
            st.error("환경변수 KIWOOM_APP_KEY / KIWOOM_APP_SECRET 가 설정되지 않았습니다."); st.stop()
        ...
        if auto_mode:
            ...
            st.session_state.s6r = results; st.session_state.s6m = ma_p; st.session_state.s6p = cd

    if st.session_state.s6r:
        results = st.session_state.s6r; map_ = st.session_state.s6m; pd_ = st.session_state.s6p
        errs = [r for r in results if r.get("error")]
        oks = [r for r in results if not r.get("error")]
        ksuf = f"{map_}_{trade_pct if not auto_mode else 'auto'}"
        if len(oks) > 1:
            _rm(results, map_, pd_, ksuf); st.markdown("---")
            for it in oks:
                with st.expander(f"📈 {it['name']} ({it['tk']})", expanded=False):
                    _r1(it["name"], it["tk"], it["enriched"], it["dets"], it["trd"], it["tlog"], map_, pd_, ksuf)
        elif len(oks) == 1:
            it = oks[0]
            _r1(it["name"], it["tk"], it["enriched"], it["dets"], it["trd"], it["tlog"], map_, pd_, ksuf)
        if errs:
