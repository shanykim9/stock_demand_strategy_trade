"""
pattern_scan6.py — 5MA 반등 + 40MA 상승 조건 추가 (v6)
═══════════════════════════════════════════════════════════════
실행: streamlit run pattern_scan6.py
...
def _analyze(days, ma_period, vol_thr, invest, price_ref, max_watch, use_bear_high, stop_loss_pct):
    ...
    def _reset():
        ...
        nonlocal signal_idx, signal_dt, avg_p, tot_cost, tot_qty, buy_dt
        state = ST_IDLE; ti_idx = 0; b_lo = b_up = 0.0; g = {}; peak_h = 0.0; det_info = {}
        signal_idx = -1; signal_dt = ""; avg_p = 0.0; tot_cost = 0.0; tot_qty = 0; buy_dt = ""
    ...
    for i in range(1, n):
        ...
        if state == ST_IDLE:
            ...
            signal_idx = -1; signal_dt = ""; avg_p = 0.0; tot_cost = 0.0; tot_qty = 0; buy_dt = ""; det_info = {}
    ...
    for e in tlog:
        ...
        if "매수시그널" in act:
            ...
            fig.add_trace(go.Scatter(x=[edt], y=[e["price"] * 1.02], mode="markers", showlegend=False,
                ...
                hovertemplate=f"%{{x|%Y-%m-%d}}<br>{act}<br>{e['price']:,.0f}<extra></extra>"), row=1, col=1)
    fig.update_layout(
        title=f"📊 {name} — 5MA 반등 v6 (MA{ma_p}+MA40)", template="plotly_dark", height=680,
